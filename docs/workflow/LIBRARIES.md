...
`shared_source_roots`, its complete file list in `shared_inputs`, and its catalog ID
in `library_ids`. The checker fails closed if any declared library input appears,
disappears, or changes outside review.
