...
a reviewed BOM. [Exact LCSC CAD sourcing](CAD_SOURCING.md) is connected; arbitrary
manufacturer-part-number lookup through other CAD providers is not connected yet.
...
| [EasyEDA/LCSC converter](https://github.com/uPesy/easyeda2kicad.py) | `easyeda2kicad --full --lcsc_id=C…` produces a symbol, footprint and available WRL/STEP models; supports KiCad 6+ and project-relative paths. | **Available, no credentials.** Pinned 1.0.1 converter with frozen source, offline conversion, checked caching and project-local library import. Its community endpoints are not a vendor service guarantee. Exact LCSC ID required; optional expected MPN rejects mismatches. Imports paired WRL; STEP alignment remains unverified. |
...
Prefer official KiCad pairs for known standard footprints. The optional pinned
EasyEDA/LCSC adapter covers exact LCSC parts without adding a provider login;
Ultra Librarian or SnapMagic can extend exact-MPN coverage after access is set up.
Ultra Librarian and SnapMagic adapters remain proposed. Other reviewed bundles
can be brought into controlled libraries using the [library policy](LIBRARIES.md).
