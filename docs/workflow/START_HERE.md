...
1. For private company work, start with [bootstrap](TEMPLATE_ADOPTION.md#bootstrap)
   ...
   Run `kicad-team template doctor --format text`, then
   `kicad-team template adopt --project-id my-hardware --format text`. The adoption command
...
3. Run `kicad-team template list --format text` to see valid project IDs,
   ...
   with `kicad-team template new-project`, choosing its ID, kind and toolchain. For an
...
5. Use a short-lived branch and run `kicad-team verify --project <id>` while working on
...
When choosing components, run `kicad-team parts --project <id> --assist`.
...
BOM, run `kicad-team parts --project <id>` and follow
...
defaults. It preserves `examples/` for independent training and acceptance checks.
...
Use `kicad-team governance-audit --format text` to inspect actual hosted
controls before the team permission and desktop rehearsals.

Before native editing, run `kicad-team check-toolchain --toolchain <toolchain-id>`.
A project-specific preflight for either Docker or an exact local KiCad CLI is
`kicad-team template doctor --native --project-id <id> --format text`.
