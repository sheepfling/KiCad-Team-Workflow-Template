...
from tools.hwrepo.contracts import parse_model_text, read_model
from tools.hwrepo.evidence import digest, source_state
...
'''
    ...
    async def test_release_readiness_and_archive_lifecycle_parity(self) -> None:
        ...
        cli_ready = await self.cli("tools.release", ReleaseReadinessReport, "check",
        ...
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            ...
            cli_package = await self.cli("tools.release", ReleasePackageReport, "package",
            ...
            for archive in (cli_archive, mcp_archive):
                with self.subTest(archive=archive.name):
                    cli_verified = await self.cli("tools.release", ReleasePackageReport, "verify",
                                                  "--archive", str(archive))
            ...
            cli_restored = await self.cli("tools.release", ReleasePackageReport, "restore",
            ...
            cli_stale = await self.cli("tools.release", ReleaseReadinessReport, "check",
            ...
            cli_denied = await self.cli_process("tools.release", "package", "--manifest",
            ...
            async with Client(create_server(self.root, allow_checks=True, allow_exports=True),
                ...
                for invalid in (False, True):
                    with self.subTest(invalid_image=invalid), patch.dict(
                        ...
                        cli = await self.cli("tools.visualize", ThreeDReport, "--project",
