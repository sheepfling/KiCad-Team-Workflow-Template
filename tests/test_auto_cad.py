...
from tools.hwrepo import auto_cad
from tools.hwrepo.cad_assets import resolve_footprint
from tools.hwrepo.contracts import read_model
from tools.hwrepo.models import AutoCadPlan, AutoCadReport, ProjectManifest
from tools.hwrepo.parts_workflow import new_receipt
...
class AutoCadTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        resolver = patch("tools.hwrepo.auto_cad._resolve", return_value=self.pair)
        resolver.start()
    ...
    def test_identical_assets_from_another_machine_preserve_historical_provenance(self) -> None:
        ...
        with patch("tools.hwrepo.auto_cad._resolve", return_value=relocated):
            report = self.plan()
    ...
    def test_tampered_plan_hash_or_source_asset_is_recalculated(self) -> None:
        ...
        with patch("tools.hwrepo.auto_cad._resolve", return_value=changed):
            self.assertEqual(self.apply(report).status, "BLOCKED")
    ...
    def test_mid_transaction_write_failure_rolls_back_every_source(self) -> None:
        ...
        with patch("tools.hwrepo.auto_cad.os.replace", side_effect=fail_once):
            result = self.apply(report)
        self.assertEqual(result.status, "BLOCKED")
