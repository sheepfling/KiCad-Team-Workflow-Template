...
class CadCliTests(unittest.TestCase):
    ...
    def invoke(self, arguments: list[str]) -> tuple[int, str]:
        stdout = StringIO()
        with (patch.object(sys, "argv", ["tools.parts", "--root", str(self.root),
    ...
    def test_source_json_passes_exact_identity_refresh_and_receipts_without_applying(self) -> None:
        with (patch("tools.parts.new_receipt", side_effect=[self.receipt, self.plan_receipt]) as receipts,
              patch("tools.hwrepo.cad_source.fetch", return_value=self.source) as fetch,
              patch("tools.hwrepo.cad_library.plan", return_value=self.planned) as plan,
    ...
    def test_text_followup_preserves_custom_root_and_quotes_every_argument(self) -> None:
        with (patch("tools.parts.new_receipt", side_effect=[self.receipt, self.plan_receipt]),
              patch("tools.hwrepo.cad_source.fetch", return_value=self.source),
              patch("tools.hwrepo.cad_library.plan", return_value=self.planned)):
        ...
        self.assertEqual(split(command), [
            "python", "-B", "-m", "tools.parts", "--root", str(self.root),
    ...
    def test_blocked_source_never_previews_or_applies_and_has_nonzero_exit(self) -> None:
        ...
        with (patch("tools.parts.new_receipt", return_value=self.receipt),
              patch("tools.hwrepo.cad_source.fetch", return_value=blocked),
              patch("tools.hwrepo.cad_library.plan") as plan,
              patch("tools.hwrepo.cad_library.apply") as apply):
    ...
    def test_usable_download_with_blocked_import_is_not_cli_success(self) -> None:
        ...
        with (patch("tools.parts.new_receipt", side_effect=[self.receipt, self.plan_receipt]),
              patch("tools.hwrepo.cad_source.fetch", return_value=self.source),
              patch("tools.hwrepo.cad_library.plan", return_value=blocked),
              patch("tools.hwrepo.cad_library.apply") as apply):
            status, stdout = self.invoke(["--source-cad", "C2040", "--format", "json"])
        self.assertEqual(status, 1)
    ...
    def test_import_uses_only_locked_plan_and_reports_applied_or_blocked_status(self) -> None:
        for applied in (True, False):
            ...
            with (self.subTest(applied=applied),
                  patch("tools.parts.new_receipt", return_value=self.receipt),
                  patch("tools.hwrepo.cad_library.apply", return_value=report) as apply,
                  patch("tools.hwrepo.cad_library.plan") as plan,
                  patch("tools.hwrepo.cad_source.fetch") as fetch):
    ...
    def test_invalid_flags_fail_before_receipts_network_or_source_changes(self) -> None:
        ...
        for flags in invalid:
            with (self.subTest(flags=flags),
                  patch("tools.parts.new_receipt") as receipt,
                  patch("tools.hwrepo.cad_source.fetch") as fetch,
                  patch("tools.hwrepo.cad_library.apply") as apply,
