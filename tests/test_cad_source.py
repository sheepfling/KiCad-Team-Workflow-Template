...
class CadSourceTests(unittest.TestCase):
    ...
    def fetch(self, supplier_id: str = "C2040", **options):
        self.counter += 1
        with (patch("tools.hwrepo.cad_source._download", side_effect=self.download),
              patch("tools.hwrepo.cad_source._converter_digest", return_value="b" * 64),
              patch("tools.hwrepo.cad_source._run_converter", side_effect=self.convert)):
    ...
    def test_cached_reuse_is_offline_and_does_not_require_converter_installation(self):
        first = self.fetch()
        with (patch("tools.hwrepo.cad_source._download", side_effect=AssertionError("network")),
              patch("tools.hwrepo.cad_source._converter_digest", side_effect=AssertionError("install"))):
    ...
    def test_missing_or_wrong_converter_gives_setup_action_without_network(self):
        ...
        for missing in [True, False]:
            with (patch("tools.hwrepo.cad_source.importlib.metadata.distribution") as distribution,
                  patch("tools.hwrepo.cad_source._download", side_effect=AssertionError("network"))):
                if missing:
    ...
    def test_download_rejects_redirect_large_empty_and_foreign_responses(self):
        ...
        with patch("tools.hwrepo.cad_source.http.client.HTTPSConnection") as connection:
