...
from tools.hwrepo.exports import command_arguments, require_declared_variant, review_artifacts_valid
from tools.hwrepo.foreign_pcb import ForeignPcbReport, convert_pcb
...
from tools.hwrepo.product import load_repository, validate_product
from tools.hwrepo.release import (
...
class ForeignPcbConversionTests(unittest.TestCase):
    ...
    def test_success_keeps_source_and_repository_untouched_until_reviewed_import(self) -> None:
        ...
        with (patch("tools.hwrepo.foreign_pcb.doctor", return_value=self.doctor_report()),
              patch("tools.hwrepo.foreign_pcb.cli_executable", return_value="/fake/kicad-cli"),
              patch("tools.hwrepo.foreign_pcb.run_command", side_effect=run)):
    ...
    def test_invalid_source_returns_json_failure_with_receipt(self) -> None:
        command = (sys.executable, "-B", "-m", "tools.template", "convert-pcb",
