...
from tools.hwrepo import releasing
from tools.hwrepo.container_git import CONTAINER_COMMON, git_metadata_mounts
from tools.hwrepo.discovery import load_config, load_registry
from tools.verify import container_command
...
class ContainerGitTests(unittest.TestCase):
    ...
    def test_standard_clone_does_not_add_mounts_or_environment(self) -> None:
        with patch("tools.hwrepo.container_git.subprocess.run") as probe:
    ...
    def test_release_validation_and_export_share_the_worktree_mapping(self) -> None:
        ...
        for export_only in (False, True):
            with (patch("tools.hwrepo.releasing.load_config", return_value=config),
                  patch("tools.hwrepo.releasing.subprocess.run", side_effect=execute)):
        ...
        self.assertIn("tools.validate", commands[0])
        self.assertIn("tools.release", commands[1])
    ...
    def test_unsupported_git_directory_relationship_is_rejected(self) -> None:
        ...
        output = f"{git_dir}\n{self.root / '.git'}\n{self.worktree}\n"
        with (patch("tools.hwrepo.container_git.subprocess.run", return_value=
    ...
    def test_unsafe_mount_delimiters_and_wrong_root_are_rejected(self) -> None:
        ...
        for output, message in (
            ...
            with (self.subTest(message=message),
                  patch("tools.hwrepo.container_git.subprocess.run", return_value=
                        subprocess.CompletedProcess([], 0, stdout=output)),
