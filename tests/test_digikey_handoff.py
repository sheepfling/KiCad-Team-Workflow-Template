...
from tools.hwrepo.contracts import parse_model
from tools.hwrepo.digikey_handoff import DigiKeyHandoffError, build_payload, send
...
class DigiKeyHandoffTests(unittest.TestCase):
    ...
    def test_single_https_post_uses_documented_noauth_schema(self) -> None:
        ...
        with patch("tools.hwrepo.digikey_handoff.http.client.HTTPSConnection",
    ...
    def test_response_trickle_cannot_reset_total_deadline(self) -> None:
        ...
        with patch("tools.hwrepo.digikey_handoff.http.client.HTTPSConnection",
                   return_value=connection), patch(
                       "tools.hwrepo.digikey_handoff.time.monotonic", side_effect=ticks,
    ...
    def test_redirects_and_server_failures_are_not_followed_or_retried(self) -> None:
        for status in (301, 302, 307, 308, 400, 429, 500):
            ...
            with self.subTest(status=status), patch(
                "tools.hwrepo.digikey_handoff.http.client.HTTPSConnection",
                return_value=connection,
    ...
    def test_invalid_local_requests_never_contact_supplier(self) -> None:
        payload = build_payload(ready())
        with patch("tools.hwrepo.digikey_handoff.http.client.HTTPSConnection") as factory:
