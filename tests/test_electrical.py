...
from tools.hwrepo.electrical import (
    bound_inputs,
    grounding_checks,
    load_analysis,
    policy_issues,
...
from tools.hwrepo.electrical_runner import analyze
from tools.hwrepo.evidence import digest
from tools.hwrepo.generation import expected_outputs
...
from tools.hwrepo.spice import (
    expanded_deck,
    measured_checks,
    run_case,
    simulation_deck,
...
class ElectricalTests(unittest.TestCase):
    ...
    def test_missing_or_wrong_simulator_does_not_pass(self) -> None:
        ...
        with patch("tools.hwrepo.spice.run_command", return_value=CommandEvidence(argv=("ngspice",), started_utc="fixture", returncode=0, stdout="ngspice-46")):
    ...
    def test_unconfigured_cli_is_explicit_nonzero_and_writes_a_receipt(self) -> None:
        root = self.stage()
        command = subprocess.run((sys.executable, "-B", "-m", "tools.electrical", "--root", str(root),
    ...
    def test_bound_inputs_and_policy_are_rechecked_after_execution(self) -> None:
        ...
        with patch("tools.hwrepo.electrical_runner.bound_inputs", side_effect=[before, {}]):
    ...
    def test_timeout_receipt_and_missing_waveforms_fail(self) -> None:
        ...
        with patch("tools.hwrepo.spice.run_command", return_value=CommandEvidence(argv=("ngspice",), started_utc="fixture", returncode=124, error="Timed out")):
    ...
    def test_portable_gate_rejects_an_overload_and_cli_cannot_bypass_model_hashes(self) -> None:
        ...
        command = subprocess.run((sys.executable, "-B", "-m", "tools.ci", "--root", str(root),
    ...
    def test_combined_verification_cannot_pass_a_failed_electrical_lane(self) -> None:
        ...
        from tools.hwrepo.models import ElectricalAnalysisReport, ElectricalCheck
        ...
        with (VerifyTests.runner_environment("10.0.0"),
              patch("tools.verify.check_all", return_value=native_summary()),
              patch("tools.hwrepo.electrical_runner.analyze", return_value=failed) as simulation):
