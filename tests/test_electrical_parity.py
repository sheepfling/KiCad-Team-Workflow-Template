...
from tools.hwrepo import electrical_setup, mcp_electrical, mcp_files
from tools.hwrepo.contracts import read_model, write_model
from tools.hwrepo.electrical import regular_input_bytes, simulation_cases
...
from tools.hwrepo.spice import expanded_deck
...
class ElectricalParityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_init_and_capture_match_cli_without_approving_requirements(self) -> None:
        ...
        process = await self.cli("tools.electrical", "--project", PROJECT, "--init")
        ...
        before = source_bytes(self.root)
        process = await self.cli("tools.electrical", "--project", PROJECT, "--capture-inputs",
    ...
    async def test_saved_native_analysis_is_root_relative_and_preserves_native_failure(self) -> None:
        ...
        process = await self.cli("tools.electrical", "--project", PROJECT, "--native-summary", summary,
        ...
        with patch.dict(os.environ, {"PATH": str(binary) + os.pathsep + os.environ.get("PATH", "")}):
            async with Client(create_server(self.mcp_root, allow_checks=True), mode="legacy") as client:
                for view, version, status in (("simulation", "47", "PASS"), ("wrong-version", "46", "FAIL")):
                    ...
                    process = await self.cli("tools.electrical", "--project", PROJECT,
    ...
    async def test_scope_selection_preserves_mixed_outcomes_like_ci(self) -> None:
        self.configured()
        process = await self.cli("tools.ci", "--electrical", "--tag", "training", "--project", PROJECT)
    ...
    async def test_doctor_and_verification_electrical_preflight_match_cli(self) -> None:
        ...
        with patch.dict(os.environ, {"PATH": str(binary) + os.pathsep + os.environ.get("PATH", "")}):
            process = await self.cli("tools.template", "doctor", "--electrical", "--project-id", PROJECT,
                                     "--runner", "local")
            self.assertEqual(process.returncode, 1, process.stderr + process.stdout)
            cli_doctor = TemplateDoctorReport.model_validate_json(process.stdout)
            process = await self.cli("tools.verify", "--project", PROJECT, "--depth", "electrical",
                                     "--runner", "local")
            self.assertEqual(process.returncode, 1, process.stderr + process.stdout)
    ...
    def test_nonregular_model_rejected_before_open_and_stale_deck_rejected(self) -> None:
        ...
        path.unlink()
        os.mkfifo(path)
        with patch("tools.hwrepo.electrical.os.open") as opened:
