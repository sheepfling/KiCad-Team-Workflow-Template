...
from tools.electrical import main as electrical_main
from tools.hwrepo.cli_output import summary
from tools.hwrepo.contracts import read_model, write_model
from tools.hwrepo.diagnostics import portable_findings
from tools.hwrepo.electrical import policy_issues, selected_config, simulation_cases
from tools.hwrepo.electrical_doctor import electrical_checks
from tools.hwrepo.electrical_runner import analyze, format_report
from tools.hwrepo.electrical_setup import capture_inputs, initialize
...
class ElectricalSetupTests(unittest.TestCase):
    ...
    def command(self, *args: str) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            (sys.executable, "-B", "-m", "tools.electrical", "--root", str(self.root),
    ...
    def test_initialization_connects_pending_requirements_and_preserves_native_expectations(self) -> None:
        ...
        with patch("tools.hwrepo.electrical_runner.capture") as native, patch("tools.hwrepo.electrical_runner.run_case") as spice:
    ...
    def test_failed_pointer_write_rolls_back_only_its_own_sidecar(self) -> None:
        ...
        with (patch("tools.hwrepo.electrical_setup.os.replace", side_effect=OSError("write blocked")),
    ...
    def test_doctor_coaches_missing_pending_and_unreviewed_configuration(self) -> None:
        ...
        initialize(self.root, PROJECT)
        with patch("tools.hwrepo.electrical_doctor.run_command") as command:
    ...
    def test_doctor_accepts_exact_multiline_banner_and_rejects_missing_wrong_or_error(self) -> None:
        ...
        for banner, code, error, status in (
            ...
            with self.subTest(banner=banner, code=code), patch("tools.hwrepo.electrical_doctor.run_command", return_value=evidence):
    ...
    def test_electrical_preflight_stops_combined_run_before_native_work(self) -> None:
        ...
        with (test_verify.VerifyTests.runner_environment("10.0.0"),
              patch("tools.hwrepo.electrical_doctor.run_command", return_value=evidence),
              patch("tools.verify.check_all") as native):
    ...
    def test_doctor_cli_routes_electrical_and_simulator_options(self) -> None:
        from tools.hwrepo.models import TemplateDoctorReport

        report = TemplateDoctorReport(native_requested=True, electrical_requested=True, checks=(), status="PASS", next_actions=())
        with (patch("tools.template.doctor", return_value=report) as doctor,
              patch.object(sys, "argv", ["tools.template", "doctor", "--electrical", "--project-id", PROJECT,
    ...
    def test_human_output_has_identity_failure_and_receipt_with_full_json_unchanged(self) -> None:
        ...
        with (patch("tools.electrical.analyze", return_value=report),
              patch.object(sys, "argv", ["tools.electrical", "--project", PROJECT, "--detail", "full"]),
    ...
    def test_standalone_example_is_valid_schema_but_has_no_approved_binding(self) -> None:
        contract = read_model(SOURCE_ROOT / "templates/electrical/contract.example.json", ElectricalAnalysisContract)
        ...
        for case in simulation_cases(contract):
            ...
            self.assertTrue((SOURCE_ROOT / "templates/electrical" / Path(case.deck).name).is_file())
    ...
    def test_hosted_workflow_uses_checked_source_and_central_gate_with_failure_artifacts(self) -> None:
        ...
        self.assertIn("tools.template doctor --electrical", workflow)
        self.assertIn("tools.ci --electrical --project", workflow)
        self.assertIn("if: always()", workflow)
        self.assertIn("build/electrical/", workflow)
        self.assertIn("tools.electrical_charts --suite", workflow)
        self.assertIn("build/electrical-charts/", workflow)
        self.assertIn("'.[charts]'", workflow)
