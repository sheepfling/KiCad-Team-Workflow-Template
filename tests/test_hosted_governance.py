...
from tools.governance_audit import main as audit_main
from tools.hwrepo.hosted_governance import ApiError, _gh, audit
...
class HostedGovernanceAuditTests(unittest.TestCase):
    ...
    def test_ruleset_controls_pass_but_unverified_people_remain_unknown(self) -> None:
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=api_response):
    ...
    def test_current_gh_repository_is_discovered_when_repo_is_omitted(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=current):
            report = audit(ROOT)
    ...
    def test_missing_legacy_controls_and_codeowners_are_needs_setup(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=missing):
            report = audit(ROOT, REPO)
        checks = {check.id: check for check in report.checks}
    ...
    def test_inaccessible_protection_does_not_prove_absence(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=inaccessible):
            report = audit(ROOT, REPO)
        checks = {check.id: check for check in report.checks}
        self.assertEqual(report.status, "UNKNOWN")
    ...
    def test_unreadable_high_priority_codeowners_cannot_be_bypassed(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=hidden):
            report = audit(ROOT, REPO)
    ...
    def test_codeowners_email_is_allowed_but_oversized_file_is_not(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=email_owner):
            report = audit(ROOT, REPO)
        self.assertEqual(next(check for check in report.checks
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=oversized):
            report = audit(ROOT, REPO)
        self.assertEqual(next(check for check in report.checks
    ...
    def test_malformed_codeowners_entry_never_passes_even_with_other_valid_owners(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=malformed):
            report = audit(ROOT, REPO)
    ...
    def test_github_codeowners_errors_and_unavailable_validation_fail_closed(self) -> None:
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=invalid_pattern):
            report = audit(ROOT, REPO)
        codeowners = next(check for check in report.checks if check.id == "codeowners_file")
        self.assertEqual(codeowners.status, "NEEDS_SETUP")
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=unavailable):
            report = audit(ROOT, REPO)
        codeowners = next(check for check in report.checks if check.id == "codeowners_file")
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=malformed_response):
            report = audit(ROOT, REPO)
        self.assertEqual(next(check for check in report.checks
                              if check.id == "codeowners_file").status, "UNKNOWN")
    ...
    def test_invalid_auth_and_governance_branch_mismatch_are_explicit(self) -> None:
        with patch("tools.hwrepo.hosted_governance._gh",
        ...
        with patch("tools.hwrepo.hosted_governance._gh", side_effect=trunk):
    ...
    def test_cli_exposes_typed_json_and_brief_text(self) -> None:
        with (
            patch("tools.hwrepo.hosted_governance._gh", side_effect=api_response),
            patch.object(sys, "argv", ["tools.governance_audit", "--root", str(ROOT),
                                       "--repo", REPO]),
