...
from tools.hwrepo import mcp_files
from tools.hwrepo.mcp_files import (
    apply_project_edit,
    artifact_path,
    list_artifacts,
...
class McpFilesTests(unittest.TestCase):
    ...
    def test_artifact_scopes_reject_sources_traversal_and_restore_payload(self) -> None:
        for name in ("build/run/log.txt", "projects/board/build/log.txt",
                     "products/system/build/report.json", "examples/projects/board/build/log.txt",
                     "examples/products/system/build/report.json"):
