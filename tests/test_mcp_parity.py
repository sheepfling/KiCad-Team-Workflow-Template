...
class McpParityTests(unittest.IsolatedAsyncioTestCase):
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(malformed_peer=broken):
                    ...
                    cli = await self.cli("tools.template", TemplateInventoryReport, "list",
    ...
    async def test_doctor_failure_parity(self) -> None:
        ...
        with patch.dict(os.environ, {"PATH": ""}):
            cli = await self.cli("tools.template", TemplateDoctorReport, "doctor", "--native",
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(missing_sheet=broken):
                    if broken:
                        self.damage_import()
                    cli = await self.cli("tools.template", ProjectImportReport, "import-project",
    ...
    async def test_import_scan_parity(self) -> None:
        ...
        cli = await self.cli("tools.template", ImportInventoryReport, "scan-imports",
    ...
    async def test_import_diagnosis_parity(self) -> None:
        self.damage_import()
        cli = await self.cli("tools.template", DiagnosticReport, "diagnose", "--source",
    ...
    async def test_project_diagnosis_parity(self) -> None:
        ...
        cli = await self.cli("tools.template", DiagnosticReport, "diagnose", "--project-id",
    ...
    async def test_rescue_parity(self) -> None:
        (self.root / "examples/projects/passive-signal-reference/project.json").write_text("{bad")
        cli = await self.cli("tools.template", LocalRescueReport, "rescue", "--project-id",
                             "controller", expected_exit=1)
        async with Client(create_server(self.root), mode="legacy") as client:
        ...
        async with Client(create_server(self.root, allow_checks=True), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(missing_source=broken):
                    if broken:
                        (self.island / "kicad/controller.kicad_sch").unlink()
                    cli = await self.cli("tools.verify", ProjectVerificationReport,
    ...
    async def test_scope_check_parity(self) -> None:
        cli = await self.cli("tools.ci", ProjectStaticPipelineReport, "--project", "controller")
        async with Client(create_server(self.root, allow_checks=True), mode="legacy") as client:
    ...
    async def test_native_scope_failure_parity(self) -> None:
        ...
        with patch.dict(os.environ, {"PATH": str(binary) + os.pathsep + os.environ.get("PATH", "")}):
            cli = await self.cli("tools.ci", CheckAllSummary, "--kicad", "--project", "controller",
    ...
    async def test_new_project_parity(self) -> None:
        cli = await self.cli("tools.template", ProjectScaffoldReport, "new-project", "--project-id",
    ...
    async def test_import_write_parity(self) -> None:
        cli = await self.cli("tools.template", ProjectImportReport, "import-project", "--source",
                             str(self.incoming), "--project-id", "incoming", "--toolchain",
                             "kicad-10.0.5")
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for tampered in (False, True):
                with self.subTest(tampered=tampered):
                    ...
                    cli = await self.cli("tools.contract_coach", ContractCoachReport, "--project-id",
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(missing_assigned_model=broken):
                    ...
                    cli = await self.cli("tools.visualize", ThreeDReport, "--project", "controller",
    ...
    async def test_parts_preferences_parity(self) -> None:
        ...
        cli = await self.cli("tools.parts", PurchasingPreferences, "--project", "controller",
        ...
        path.unlink()
        cli_created = await self.cli("tools.parts", McpPurchasingPreferencesResult,
        ...
        async with Client(create_server(self.root, allow_edits=True), mode="legacy") as client:
            ...
            cli_updated = await self.cli("tools.parts", McpPurchasingPreferencesResult,
            ...
            rejected_cli = await self.cli_process(
                "tools.parts", "--project", "controller", "--save-preferences", "--boards", "30",
                "--expected-sha256", created.after_sha256,
            )
    ...
    async def test_parts_quantities_and_native_failure_parity(self) -> None:
        ...
        cli = await self.cli("tools.parts", PurchasingReport, "--project", "controller",
