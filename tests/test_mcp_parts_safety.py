...
from tools.hwrepo import mcp_files, mcp_parts, parts_workflow
...
from tools.hwrepo.models import McpPurchasingPreferencesResult, PurchasingPreferences
...
class McpPartsSafetyTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_tool_arguments_cannot_enable_capture_or_choose_a_destination(self) -> None:
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            ...
            with patch("tools.hwrepo.parts_workflow.prepare") as prepare:
    ...
    def test_preferences_creation_and_update_refuse_concurrent_source_changes(self) -> None:
        ...
        with (patch("tools.hwrepo.parts_workflow.init_preferences", side_effect=create_competing),
        ...
        with (patch("tools.hwrepo.mcp_files.apply_project_edit", side_effect=change_before_apply),
