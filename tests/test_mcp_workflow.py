...
class McpWorkflowTests(unittest.TestCase):
    ...
    def test_receipt_parent_links_and_artifact_scope_fail_before_execution(self) -> None:
        with patch("tools.hwrepo.mcp_workflow.diagnostics.diagnose_project") as execute:
    ...
    def test_diagnostic_crash_is_recorded_and_propagated(self) -> None:
        with (patch("tools.hwrepo.mcp_workflow.diagnostics.diagnose_project",
    ...
    def test_contract_capture_uses_fixed_runner_and_preserves_contract(self) -> None:
        ...
        with patch("tools.hwrepo.mcp_workflow.contract_coach.LocalNetlistRunner", return_value=runner) as create:
    ...
    def test_prepare_review_reuses_real_candidate_gate_with_synthetic_native_evidence(self) -> None:
        ...
        with (patch("tools.hwrepo.mcp_workflow.doctor", return_value=runner_report()),
              patch("tools.hwrepo.mcp_workflow.cli_executable", return_value="kicad-cli"),
              patch("tools.hwrepo.releasing.run_native", side_effect=copy_native)):
    ...
    def test_container_prepare_captures_progress_and_reads_retained_manifest(self) -> None:
        ...
        def run(root, argv, timeout):
            self.assertIn("tools.release", argv)
        ...
        with (patch("tools.hwrepo.mcp_workflow.doctor", return_value=runner_report("container")),
              patch("tools.hwrepo.mcp_workflow.run_command", side_effect=run)):
    ...
    def test_export_checks_settings_clean_source_and_fresh_paths(self) -> None:
        with patch("tools.hwrepo.mcp_workflow.selected_cli") as runner:
    ...
    def test_export_executes_real_exporter_with_explicit_synthetic_native_outputs(self) -> None:
        ...
        with (patch("tools.hwrepo.mcp_workflow.doctor", return_value=runner_report()),
              patch("tools.hwrepo.mcp_workflow.cli_executable", return_value="kicad-cli"),
              patch("tools.hwrepo.exports.execute", side_effect=execute)):
    ...
    def test_subprocess_failure_preserves_output_without_polluting_protocol(self) -> None:
        ...
        with patch("tools.hwrepo.mcp_workflow.run_command") as run:
            with self.assertRaisesRegex(ValueError, "already exists"):
    ...
    def test_container_dependency_symlink_fails_before_any_export_write(self) -> None:
        ...
        try:
            ...
            self.skipTest("Directory symlinks unavailable")
        with (patch("tools.hwrepo.mcp_workflow.selected_cli", return_value=None),
              patch("tools.hwrepo.mcp_workflow.run_command") as run,
