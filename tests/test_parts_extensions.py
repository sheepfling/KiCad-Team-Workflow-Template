...
from tools.hwrepo import mcp_parts_extensions, part_picker, supplier_handoff
from tools.hwrepo.cad_download import fetch_official_footprint
...
from tools.hwrepo.parts_workflow import save_report
...
class PartsExtensionsTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def cli(self, model, *arguments, expected_exit=0, stub_http=False):
        command = [sys.executable, "-B", "-m", "tools.parts"]
        if stub_http:
            ...
            script = (
                ...
                "sys.argv=['tools.parts',*sys.argv[1:]]; "
                "p=patch('tools.hwrepo.digikey_handoff._post',return_value="
                + repr(json.dumps(_REVIEW_URL).encode())
                + "); p.start(); runpy.run_module('tools.parts',run_name='__main__')"
            )
    ...
    async def test_picker_and_edit_capabilities_and_stale_evidence(self) -> None:
        ...
        async with Client(create_server(self.root, allow_exports=True)) as client:
            with patch("tools.hwrepo.contract_coach.capture") as capture:
    ...
    async def test_auto_cad_cli_protocol_parity(self) -> None:
        ...
        with patch.dict(os.environ, {"KICAD10_FOOTPRINT_DIR": str(member.parent.parent),
            ...
            async with Client(create_server(self.root, allow_exports=True, allow_edits=True)) as client:
                ...
                with patch("tools.hwrepo.mcp_parts_extensions._output", side_effect=replace_after_hash):
    ...
    async def test_download_permission_and_cache_symlink_boundaries(self) -> None:
        ...
        with patch("tools.hwrepo.cad_download._download") as transfer:
        ...
        fixture.urls = []
        with patch("tools.hwrepo.cad_download._download", side_effect=fixture.download):
    ...
    async def test_supplier_prepare_and_submit_cli_protocol_parity(self) -> None:
        ...
        with patch("tools.hwrepo.digikey_handoff._post", side_effect=AssertionError("offline preparation")):
        ...
        with patch("tools.hwrepo.digikey_handoff._post", return_value=json.dumps(_REVIEW_URL).encode()) as post:
        ...
        with patch("tools.hwrepo.digikey_handoff._post", side_effect=AssertionError("must not resubmit")):
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
    ...
    async def test_supplier_capability_digest_and_stale_review_reject_before_network(self) -> None:
        ...
        with patch("tools.hwrepo.digikey_handoff._post") as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
    ...
    async def test_supplier_uncertain_attempt_is_persistent_and_never_retried(self) -> None:
        ...
        prepared = await self.prepared_handoff()
        arguments = {"handoff": prepared.handoff, "expected_sha256": prepared.handoff_sha256}
        with patch("tools.hwrepo.digikey_handoff._post", side_effect=OSError("synthetic disconnect")) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
        ...
        with patch("tools.hwrepo.digikey_handoff._post", side_effect=AssertionError("must never retry")) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
    ...
    async def test_supplier_attempt_is_reserved_before_post_and_source_drift_remains_blocked(self) -> None:
        ...
        with patch("tools.hwrepo.digikey_handoff._post", side_effect=external_reply) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
                result = await self.call(client, "submit_supplier_handoff", SupplierHandoffReport,
                    handoff=prepared.handoff, expected_sha256=prepared.handoff_sha256)
            self.assertEqual(post.call_count, 1)
    ...
    async def test_part_apply_uses_reviewed_bytes_when_original_map_is_swapped(self) -> None:
        ...
        with patch("tools.hwrepo.mcp_parts_extensions._output", side_effect=swap_after_review):
        ...
        self.assertEqual(applied.status, "APPLIED", applied.issues)
        from tools.hwrepo.part_cad import read_cad_components
    ...
    async def test_supplier_digest_race_cannot_substitute_another_valid_payload(self) -> None:
        ...
        with (patch("tools.hwrepo.supplier_handoff.digest", side_effect=swap_around_digest),
              patch("tools.hwrepo.digikey_handoff._post", return_value=json.dumps(_REVIEW_URL).encode()) as post):
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
        ...
        async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
            for variant in variants:
                with self.subTest(variant=variant):
                    ...
                    with (patch("tools.hwrepo.digikey_handoff._post") as post,
                          patch("tools.hwrepo.supplier_handoff.read_model", wraps=read_model) as reader):
                        result = await client.call_tool("submit_supplier_handoff", {
                            "handoff": prepared.handoff, "expected_sha256": prepared.handoff_sha256})
