...
from tools import release as release_cli
...
class ReviewScopeParityTests(unittest.IsolatedAsyncioTestCase):
    ...
    def cli(self, *arguments: str) -> tuple[int, str]:
        output = StringIO()
        with (patch.object(sys, "argv", ["tools.release", "prepare", "--root", str(self.root),
    ...
    async def test_actual_cli_and_mcp_prepare_same_project_variant_scope(self) -> None:
        ...
        with (patch("tools.hwrepo.releasing.run_native", side_effect=self.synthetic_native),
              patch("tools.hwrepo.mcp_workflow.selected_cli", return_value="kicad-cli")):
    ...
    def test_container_scope_preserves_selection_and_captures_nonprotocol_stdout(self) -> None:
        ...
        from tools.hwrepo.releasing import resolve_variants
        ...
        with (patch("tools.hwrepo.mcp_workflow.selected_cli", return_value=None),
              patch("tools.hwrepo.mcp_workflow.run_command", side_effect=command),
