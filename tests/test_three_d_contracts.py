...
from tools.hwrepo.three_d import generate
...
class ThreeDContractTests(unittest.TestCase):
    ...
    def test_real_inspection_receipts_round_trip_through_canonical_contracts(self) -> None:
        with patch("tools.hwrepo.three_d.doctor") as native:
    ...
    def test_population_report_never_authorizes_build_or_skips_required_checks(self) -> None:
        ...
        report = ModelPopulationReport(
            ...
            next_commands=(f"python -B -m tools.verify --project {PROJECT} --depth native",),
        )
    ...
    def test_linked_derived_board_is_rejected_before_inspection_or_native_execution(self) -> None:
        ...
        with (
            patch("tools.hwrepo.model_inventory._children") as parse,
        ...
        with (
            patch("tools.hwrepo.three_d.load_config", return_value=self.config),
            patch("tools.hwrepo.three_d.inspect_models") as inspect,
            patch("tools.hwrepo.three_d._run_kicad") as run_native,
        ):
