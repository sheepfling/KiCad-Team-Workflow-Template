...
from tools.hwrepo.diagnostic_journal import DiagnosticJournal
from tools.hwrepo.diagnostics import diagnose_project
from tools.hwrepo.model_inventory import ModelInventoryReport
from tools.hwrepo.models import ProjectKind, ProjectManifest, ProjectTestContract
...
class ThreeDDiagnosticTests(unittest.TestCase):
    ...
    def test_missing_models_are_review_findings_and_do_not_fail_portable_diagnosis(self) -> None:
        ...
        with (patch("tools.hwrepo.three_d.doctor", side_effect=AssertionError("native runner started")),
              patch("tools.hwrepo.three_d.generate", side_effect=AssertionError("3D export started"))):
    ...
    def test_missing_model_file_blocks_the_same_repair_queue(self) -> None:
        ...
        self.assertIn("tools.template diagnose", result.next_command)
        self.assertEqual(self.board.read_bytes(), original)

    def test_non_pcb_diagnosis_does_not_attempt_model_inspection(self) -> None:
        with patch("tools.hwrepo.diagnostics.inspect_models", side_effect=AssertionError("not a PCB")):
    ...
    def test_model_read_error_preserves_other_diagnostic_findings(self) -> None:
        with patch("tools.hwrepo.diagnostics.inspect_models", side_effect=ValueError("Linked board path")):
            result = diagnose_project(self.root, "controller")
        self.assertEqual(result.status, "NEEDS_WORK")
