...
from tools.hwrepo.models import CommandEvidence, EnvironmentCheck, TemplateDoctorReport
from tools.hwrepo.three_d import ThreeDReport, generate
...
class VisualizeTests(unittest.TestCase):
    ...
    def test_broken_model_blocks_export_before_runner_selection(self) -> None:
        ...
        with patch("tools.hwrepo.three_d.doctor", side_effect=AssertionError("runner started")):
    ...
    def test_wrong_native_version_and_command_failure_keep_actionable_evidence(self) -> None:
        ...
        with patch("tools.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "tools.hwrepo.three_d._run_kicad", side_effect=wrong_version,
        ):
        ...
        observed.clear()
        with patch("tools.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "tools.hwrepo.three_d._run_kicad", side_effect=failed_render,
        ):
    ...
    def test_success_exit_with_invalid_image_is_not_a_successful_export(self) -> None:
        ...
        with patch("tools.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "tools.hwrepo.three_d._run_kicad", side_effect=invalid_image,
        ):
    ...
    def test_successful_exports_are_hashed_and_source_rewrites_invalidate_them(self) -> None:
        ...
        original = self.board.read_bytes()
        with patch("tools.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "tools.hwrepo.three_d._run_kicad", side_effect=fake_export,
        ):
        ...
        with patch("tools.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "tools.hwrepo.three_d._run_kicad", side_effect=mutating_export,
        ):
