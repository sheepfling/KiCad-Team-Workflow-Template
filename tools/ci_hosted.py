...
def markdown_checks(root: Path, log: HostedLog) -> None:
    log.run("docs-policy", (sys.executable, "-B", "-m", "tools.docs_policy"), cwd=root)
...
def portable_lane(root: Path, scope: str, projects: tuple[str, ...],
    ...
    if scope == "docs":
        ...
        argv = (sys.executable, "-B", "-m", "tools.ci", *(
        ...
        log.run("portable-full", (sys.executable, "-B", "-m", "tools.ci",
...
def windows_smoke(root: Path, log: HostedLog) -> None:
    log.run("windows-inventory", (sys.executable, "-B", "-m", "tools.template", "list",
...
def native_lane(root: Path, *, project: str, image: str, pr_head: str,
    ...
    try:
        ...
        log.run("native-deps", (sys.executable, "-B", "-m", "tools.native_deps",
                                "--image", image), cwd=root)
        log.run("native-check", (*docker,
                                 'python3 -m tools.ci --kicad --project "$PROJECT_ID" '
                                 + "--output build/review"), cwd=root, env=environment)
        if fault_probes:
            log.run("fault-probes", (*docker, "python3 -m tools.ci --fault-probes --output build/fault-probes"), cwd=root, env=environment)
    finally:
...
def release_lane(root: Path, log: HostedLog) -> None:
    ...
    shutil.copytree(reference_root(), target, ignore=shutil.ignore_patterns(".git"))
    ...
    for stage, command in (
        ("release-prepare", ("tools.release", "prepare", "--project",
                             "arduino-uno-status-led", "--release-id", "ci-review")),
        ("release-package", ("tools.release", "package", "--manifest",
                             "build/releases/ci-review/manifest.json", "--output", "build/ci-review.zip")),
        ("release-verify", ("tools.release", "verify", "--archive", "build/ci-review.zip")),
    ):
    ...
    imported_path = log.run("foreign-import", (
        sys.executable, "-B", "-m", "tools.template", "import-project", "--source",
