...
def selection(root: Path, project_id: str, map_path: Path, output: Path,
    ...
    try:
        ...
        commands = ((f"python -B -m tools.parts --project {project_id} --sync-models",) if pending else (
            f"python -B -m tools.verify --project {project_id} --depth native",
            f"python -B -m tools.visualize --project {project_id}",
            f"python -B -m tools.parts --project {project_id}",
        )) if apply else (
            f"python -B -m tools.parts --project {project_id} --selection {shlex.quote(str(locked_path))} --apply",
        )
