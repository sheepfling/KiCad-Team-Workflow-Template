...
def selected_project(root: Path, project_id: str) -> ProjectRecord:
    ...
    if project is None:
        raise ValueError(f"Unknown project {project_id!r}; run tools.template list --format text")
    return project
...
def prepare(
    ...
    try:
        ...
        return PurchasingReport(
            ...
            next_actions=(
                "Fix the input or runner problem above and rerun tools.parts.",
                (f"Check the runner with: python -B -m tools.template doctor --native "
